//! `absd`, the resident Absentia service.
//!
...
use absentia_audit::AuditLog;
use absentia_auth::audit_events as ev;
use absentia_core::state::DeadmanState;
use absentia_core::time::{self, TimeAnchor};
use absentia_core::{Assurance, VaultId};
use absentia_destroy::{authorize, execute, needs_resume, Trigger};
use absentia_journal::{AuditAnchor, StateJournal};
use absentia_policy::DeadmanPolicy;
use absentia_presence::{PresenceConfig, PresenceEngine, SignalKind};
use absentia_audit::AuditLog as ServiceAuditLog;
use absentia_platform::watch;
use absentia_sanitize::SanitizationProfile;

const USAGE: &str = "\
Apex Absentia service

  absd watch <vault> [options]     evaluate the deadman policy continuously
  absd once  <vault> [options]     evaluate once and exit
...
fn main() -> ExitCode {
    ...
    if args.is_empty() || args[0] == "--help" || args[0] == "-h" {
        println!("Apex Absentia service {}", env!("CARGO_PKG_VERSION"));
        println!("{USAGE}");
    ...
    let once = match args[0].as_str() {
        ...
        "once" => true,
        other => {
            eprintln!("absd: unknown command {other}\n\n{USAGE}");
            return ExitCode::FAILURE;
        }
    };
    let Some(vault) = args.get(1).map(PathBuf::from) else {
        eprintln!("absd: a vault path is required");
        return ExitCode::FAILURE;
    };
    ...
    match ServiceAuditLog::open(audit_path(vault), id) {
        Ok(mut log) => {
            if log.append(event, detail).is_err() {
                eprintln!("absd: warning: could not write an audit record");
            }
        }
        Err(e) => eprintln!("absd: warning: could not open the audit log: {e}"),
    }
}
...
fn presence_from_audit(log: &AuditLog, engine: &mut PresenceEngine) -> absentia_core::Result<()> {
...
fn run(cfg: Config) -> absentia_core::Result<ExitCode> {
    ...
    println!("Absentia service watching {}", cfg.vault.display());
...
fn tick(cfg: &Config, origin: Instant, reported: &mut Reported) -> absentia_core::Result<Tick> {
    ...
    if let absentia_core::time::ClockAnomaly::Rollback { seconds } = clock {
    ...
    if computed == DeadmanState::Armed {
        ...
        if cfg.dry_run {
            ...
            for (stage, detail) in absentia_destroy::simulate(&cfg.vault, cfg.profile) {
