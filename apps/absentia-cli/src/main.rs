//! `abs`, the Apex Absentia command line.
//!
...
use absentia_core::{Assurance, Error, Result};
use absentia_crypto::CryptoSuite;
use absentia_kdf::KdfParams;
use absentia_audit::{AuditLog, ChainStatus};
use absentia_core::state::DeadmanState;
use absentia_core::time::{self, TimeAnchor};
use absentia_destroy::{authorize_explicit, execute, needs_resume, simulate, Trigger};
use absentia_journal::{AuditAnchor, AuditAnchorStatus, JournalStatus, StateJournal};
use absentia_enterprise::remote::SigningIdentity;

use absentia_platform::service::{ServicePlan, Supervisor};
use absentia_split::providers::ComponentProvider;
use absentia_remote::custodian::{Custodian, DirectoryCustodian, HeldShare};
use absentia_remote::{signing_identity, verifying_key, Intent, Request, Verdict};
use absentia_split::{assess, ComponentKind, RecoveryToken, SplitBundle, UserProvider};
use absentia_platform::{storage_report, supervision_status_label};
use absentia_sanitize::SanitizationProfile;
use absentia_policy::DeadmanPolicy;
use absentia_presence::{PresenceConfig, PresenceEngine, SignalKind};
use absentia_auth::{audit_events as ev, FactorKind, FactorSet};
use absentia_vault::{Vault, VaultOptions};

const USAGE: &str = "\
Apex Absentia - encrypted vault

  abs vault create <path>            create a vault
  abs vault status <path>            show vault metadata
  abs vault verify <path>            verify integrity of every chunk
  abs vault list   <path>            list stored files
  abs vault import <path> <file>...  add files
  abs vault export <path> <dir>      extract everything

  abs auth list    <path>            show required authentication factors
  abs auth register-fido <path>      bind a FIDO2 authenticator

  abs audit show   <path>            show the audit log
  abs audit verify <path>            check the audit chain

  abs deadman status   <path>        presence, countdown and state
  abs deadman checkin  <path>        record an explicit check-in
  abs deadman configure <path>       show or set the policy
  abs journal verify   <path>        check the state journal
  abs security audit                 report what this build enforces
  abs platform report <path>         what this filesystem allows
  abs enterprise keygen              generate an organization signing key
  abs recovery explain               how threshold recovery behaves

  abs split enroll <path> <token> <custodian>
                                    protect a vault with split keys
  abs split custody <path> <dir>     give the third component to a custodian
  abs duress set <path> <file>...    set a duress password showing these files
  abs split checkin <path>           tell a custodian you are still here
  abs split status <path>            split-key protection for this vault
  abs split explain                  the component model and what it defends
  abs service unit    <path>         print a service definition to review

  abs destroy status  <path>         destruction state
  abs destroy dry-run <path>         simulate destruction, changing nothing
  abs panic lock      <path>         close sessions, leave the vault intact
  abs panic destroy   <path>         irreversibly destroy the vault
...
Destruction is real and irreversible. `abs panic destroy` overwrites the key,
and the deadman policy can do the same through `absd` when it is started with
--allow-destruction. Run `abs destroy dry-run` first.
";
...
fn run(args: &[String]) -> Result<ExitCode> {
    if args.is_empty() || args[0] == "--help" || args[0] == "-h" {
        println!("Apex Absentia {}", env!("CARGO_PKG_VERSION"));
        println!("{USAGE}");
    ...
    match (args[0].as_str(), args.get(1).map(|s| s.as_str())) {
        ...
        ("auth", _) => {
            ...
            println!("composition layer exists and is tested; see `abs auth list`.");
            Ok(ExitCode::SUCCESS)
        }
        _ => {
            eprintln!("{USAGE}");
...
fn cmd_create(args: &[String]) -> Result<ExitCode> {
    ...
    let path = pos.first().ok_or_else(|| Error::Other("usage: abs vault create <path>".into()))?;
    ...
    if args.iter().any(|a| a == "--store") {
        opts.compress_suite = absentia_compress::CompressSuite::Store;
    }
    ...
    println!("At least {} characters. Four or five unrelated words joined by dashes is",
        absentia_kdf::strength::MIN_LENGTH);
    ...
    absentia_kdf::strength::require_acceptable(&password)?;
...
fn record(vault_path: &str, vault_id: absentia_core::VaultId, event: &str, detail: &str) {
    ...
    if let Ok(mut log) = AuditLog::open(audit_path(vault_path), vault_id) {
        if log.append(event, detail).is_err() {
            eprintln!("abs: warning: could not write the audit record");
        }
    }
}
...
fn open_split_vault(path: &str, token_paths: &[String], want_password: bool) -> Result<Vault> {
    ...
    if want_password {
        ...
        // Tried before the components are composed. Under coercion the person
        // may have a token attached or none at all, and a duress password has
        // to work either way.
        if let Some(v) = Vault::try_duress_open(path, password.as_bytes())? {
            return Ok(v);
        }
    ...
    match Vault::open_with_components(path, &components) {
        ...
        Err(e) => {
            record(path, absentia_core::VaultId::nil(), ev::AUTH_FAILURE, "split");
            Err(e)
        }
    }
}
...
fn open_vault_with_args(path: &str, args: &[String]) -> Result<Vault> {
    if vault_is_split_protected(path).unwrap_or(false) {
        // The duress identity is tried before a token is demanded. Under
        // coercion the person may not have their token, and a duress password
        // that only worked on unprotected vaults would do nothing on exactly
        // the vaults most likely to need it.
        //
        // The same fix was made in the boundary the window uses. This path is
        // the command line's own, and it was missed, which is the third time a
        // rule written in one place has not reached the other.
        let tokens: Vec<String> = args
            ...
            .collect();
        if tokens.is_empty() {
            // A duress password opens a split vault without a token, so it is
            // tried before the requirement is reported.
            let password = prompt_password(false)?;
            if let Some(v) = Vault::try_duress_open(path, password.as_bytes())? {
                return Ok(v);
            }
...
fn vault_is_split_protected(path: &str) -> Result<bool> {
    ...
    let mut hb = [0u8; absentia_format::HEADER_LEN];
    f.read_exact(&mut hb).map_err(|_| Error::Format("file is too short".into()))?;
    let header = absentia_format::Header::from_bytes(&hb)?;
    Ok(header.flags & absentia_format::flags::SPLIT_PROTECTED != 0)
}
...
fn open_vault(path: &str) -> Result<Vault> {
    ...
    match Vault::open(path, password.as_bytes()) {
        ...
        Err(e) => {
            ...
            record(path, absentia_core::VaultId::nil(), ev::AUTH_FAILURE, "");
            Err(e)
        }
    }
}
...
fn cmd_status(args: &[String]) -> Result<ExitCode> {
    let path = args.first().ok_or_else(|| Error::Other("usage: abs vault status <path>".into()))?;
    let v = open_vault_with_args(path, args)?;
    let total: u64 = v.entries().iter().map(|e| e.size).sum();
    println!("APEX ABSENTIA");
    println!();
...
fn cmd_list(args: &[String]) -> Result<ExitCode> {
    let path = args.first().ok_or_else(|| Error::Other("usage: abs vault list <path>".into()))?;
    let v = open_vault_with_args(path, args)?;
...
fn cmd_verify(args: &[String]) -> Result<ExitCode> {
    let path = args.first().ok_or_else(|| Error::Other("usage: abs vault verify <path>".into()))?;
    let v = open_vault_with_args(path, args)?;
    let r = v.verify()?;
...
fn cmd_import(args: &[String]) -> Result<ExitCode> {
    if args.len() < 2 {
        return Err(Error::Other("usage: abs vault import <vault> <file>...".into()));
    }
...
fn cmd_export(args: &[String]) -> Result<ExitCode> {
    if args.len() < 2 {
        return Err(Error::Other("usage: abs vault export <vault> <dir>".into()));
    }
...
fn cmd_auth_list(args: &[String]) -> Result<ExitCode> {
    let path = args.first().ok_or_else(|| Error::Other("usage: abs auth list <path>".into()))?;
    let v = open_vault_with_args(path, args)?;
...
fn cmd_audit_show(args: &[String]) -> Result<ExitCode> {
    let path = args.first().ok_or_else(|| Error::Other("usage: abs audit show <path>".into()))?;
    let log = AuditLog::open(audit_path(path), absentia_core::VaultId::nil())?;
...
fn cmd_audit_verify(args: &[String]) -> Result<ExitCode> {
    let path = args.first().ok_or_else(|| Error::Other("usage: abs audit verify <path>".into()))?;
    let log = AuditLog::open(audit_path(path), absentia_core::VaultId::nil())?;
    println!("AUDIT CHAIN");
    println!();
    match log.verify()? {
...
fn cmd_deadman_status(args: &[String]) -> Result<ExitCode> {
    let path = args.first().ok_or_else(|| Error::Other("usage: abs deadman status <path>".into()))?;
    let (policy, confidence, since_strong, state) = deadman_snapshot(path)?;
    let journal = StateJournal::open(journal_path(path), absentia_core::VaultId::nil())?;
...
fn cmd_deadman_checkin(args: &[String]) -> Result<ExitCode> {
    let path = args.first().ok_or_else(|| Error::Other("usage: abs deadman checkin <path>".into()))?;
    ...
    if let Err(e) = journal.record(state, now, deadline, confidence, anchor) {
        eprintln!("abs: warning: could not record the state transition: {e}");
    }
...
fn cmd_deadman_configure(args: &[String]) -> Result<ExitCode> {
    let path = args.first().ok_or_else(|| {
        Error::Other("usage: abs deadman configure <path> [--enable|--disable] [--timeout SECONDS]".into())
    })?;
...
",
        ...
        record(path, absentia_core::VaultId::nil(), ev::POLICY_CHANGED, "");
    }
...
fn cmd_journal_verify(args: &[String]) -> Result<ExitCode> {
    let path = args.first().ok_or_else(|| Error::Other("usage: abs journal verify <path>".into()))?;
    let journal = StateJournal::open(journal_path(path), absentia_core::VaultId::nil())?;
    let log = AuditLog::open(audit_path(path), absentia_core::VaultId::nil())?;
...
fn cmd_destroy_status(args: &[String]) -> Result<ExitCode> {
    let path = args.first().ok_or_else(|| Error::Other("usage: abs destroy status <path>".into()))?;
    let journal = StateJournal::open(journal_path(path), absentia_core::VaultId::nil())?;
    ...
    if needs_resume(&journal) {
        ...
        println!("  Run `absd once {path} --allow-destruction` to finish it.");
    }
...
fn cmd_destroy_dryrun(args: &[String]) -> Result<ExitCode> {
    let path = args.first().ok_or_else(|| Error::Other("usage: abs destroy dry-run <path>".into()))?;
    ...
    for p in absentia_destroy::sidecar_paths(std::path::Path::new(path)) {
        println!("  {}", p.display());
    }
    Ok(ExitCode::SUCCESS)
}
...
fn cmd_panic_lock(args: &[String]) -> Result<ExitCode> {
    let path = args.first().ok_or_else(|| Error::Other("usage: abs panic lock <path>".into()))?;
    ...
    println!();
    println!("  This operation never destroys data. For that, see `abs panic destroy`.");
    let _ = path;
    Ok(ExitCode::SUCCESS)
}

fn cmd_panic_destroy(args: &[String]) -> Result<ExitCode> {
    let path = args.first().ok_or_else(|| Error::Other("usage: abs panic destroy <path>".into()))?;
...
fn user_component(path: &str, password: &str) -> Result<absentia_secure_memory::Key256> {
    use std::io::Read;
    let mut f = std::fs::File::open(path)?;
    let mut hb = [0u8; absentia_format::HEADER_LEN];
    f.read_exact(&mut hb)
...
fn cmd_split_enroll(args: &[String]) -> Result<ExitCode> {
    let path = args
        .first()
        .ok_or_else(|| Error::Other("usage: abs split enroll <vault> <token-file>".into()))?;
    let token_path = args
        .get(1)
    ...
    println!("  Split bundle   {}", absentia_vault::split_bundle_path(std::path::Path::new(path)).display());
...
fn cmd_duress_set(args: &[String]) -> Result<ExitCode> {
    let path = args
        .first()
        .ok_or_else(|| Error::Other("usage: abs duress set <vault> <file>...".into()))?;
    // The value after a flag is that flag's, not a decoy file. Filtering only
    // on a leading "--" swallowed the path after --token, which meant the
    // decoy vault would have shown the recovery token itself: the one file it
    // must never contain.
    ...
    for a in &args[1..] {
        ...
        if a.starts_with("--") {
            ...
            if matches!(a.as_str(), "--token" | "--custodian" | "--timeout") {
                skip_next = true;
            }
            continue;
        }
    ...
    // Through the token-aware path, because a split-protected vault cannot be
    // opened with a password alone and setting a duress password on one is
    // exactly the case somebody would want.
    let mut v = if vault_is_split_protected(path).unwrap_or(false) {
        let tokens: Vec<String> = args
            .iter()
            .enumerate()
            .filter(|(_, a)| *a == "--token")
        ...
        if tokens.is_empty() {
            return Err(Error::Other(format!(
                "{path} is split protected; pass --token <file> as well as the password"
            )));
        }
        let mut components = vec![(ComponentKind::User, user_component(path, &real)?)];
        for t in &tokens {
            let token = RecoveryToken::decode(&std::fs::read_to_string(t)?)?;
            components.push((ComponentKind::Remote, token.key()?));
            components.push((ComponentKind::Custodian, token.key()?));
        }
        Vault::open_with_components(path, &components)?
    } else {
        Vault::open(path, real.as_bytes())?
    };
    println!("Now choose the duress password.");
    let duress = prompt_password(true)?;
    absentia_kdf::strength::require_acceptable(&duress)?;
...
fn cmd_split_custody(args: &[String]) -> Result<ExitCode> {
    let path = args
        .first()
        .ok_or_else(|| Error::Other("usage: abs split custody <vault> <custodian-dir>".into()))?;
    let dir = args
        .get(1)
    ...
    let bundle = SplitBundle::decode(&std::fs::read(&sp).map_err(|_| {
        Error::Other("this vault is not split protected; run `abs split enroll` first".into())
    })?)?;
    ...
    println!();
    println!("  Check in with:  abs split checkin {path}");
    Ok(ExitCode::SUCCESS)
}
...
fn cmd_split_checkin(args: &[String]) -> Result<ExitCode> {
    let path = args
        .first()
        .ok_or_else(|| Error::Other("usage: abs split checkin <vault> --custodian <dir>".into()))?;
...
fn header_kdf(path: &str) -> Result<([u8; 32], absentia_kdf::KdfParams)> {
    use std::io::Read;
    let mut f = std::fs::File::open(path)?;
    let mut hb = [0u8; absentia_format::HEADER_LEN];
    f.read_exact(&mut hb)
        .map_err(|_| Error::Format("file is too short to be an AZV vault".into()))?;
    let h = absentia_format::Header::from_bytes(&hb)?;
    Ok((h.kdf_salt, h.kdf_params))
}

fn cmd_split_status(args: &[String]) -> Result<ExitCode> {
    let path = args.first().ok_or_else(|| Error::Other("usage: abs split status <path>".into()))?;
    let sp = split_path(path);
    ...
    if !sp.exists() {
        ...
        println!();
        println!("  Run `abs split explain` for what enrollment would change.");
        return Ok(ExitCode::SUCCESS);
    }
...
fn cmd_recovery_explain() -> Result<ExitCode> {
    ...
    println!();
    println!("  Minimum threshold  {}", absentia_enterprise::recovery::MIN_THRESHOLD);
    println!("  Maximum custodians {}", absentia_enterprise::recovery::MAX_CUSTODIANS);
    println!();
...
fn cmd_platform_report(args: &[String]) -> Result<ExitCode> {
    ...
    let snapshots = absentia_platform::snapshot_exposure(std::path::Path::new(&path));
...
fn cmd_service_unit(args: &[String]) -> Result<ExitCode> {
    let vault = args
        .first()
        .ok_or_else(|| Error::Other("usage: abs service unit <vault> [--allow-destruction]".into()))?;

    let exe = std::env::current_exe()
        .map(|p| p.with_file_name("absd"))
        .unwrap_or_else(|_| PathBuf::from("absd"));
    ...
    eprintln!("#");
    eprintln!("# Absentia does not install this itself: writing to a service directory");
...
fn cmd_audit() -> Result<ExitCode> {
    let (mem, note) = absentia_secure_memory::probe_memory_protection();
    ...
    println!("SECURITY AUDIT - Apex Absentia {}", env!("CARGO_PKG_VERSION"));
    println!();
