//! Apex Absentia desktop window.
//!
//! A native front end over `absentia-ipc`. It contains no cryptography, no
...
use absentia_vault::VaultOptions;
...
struct App {
    ...
    tokens: Vec<PathBuf>,
    use_password: bool,
    split: Option<absentia_ipc::SplitStatusView>,
    watch: Option<absentia_ipc::WatchView>,
    autostart: Option<absentia_ipc::AutostartView>,
    custody: Option<absentia_ipc::CustodyView>,
    ...
    overviews: Vec<absentia_ipc::VaultOverview>,
...
impl App {
    ...
    fn poll(&mut self) {
        ...
        self.overviews = absentia_ipc::remembered_vaults();
    ...
    fn open_vault(&mut self, path: PathBuf) {
        self.clear_vault_state();
        absentia_ipc::remember_vault(&path);
...
impl App {
    fn top_rail(&mut self, ctx: &egui::Context) {
        egui::TopBottomPanel::top("rail")
            ...
            .show(ctx, |ui| {
                ui.horizontal(|ui| {
                    ui.label(
                        egui::RichText::new("APEX ABSENTIA").size(15.0).strong().color(TEXT),
                    );
    ...
    fn vault_list(&mut self, ui: &mut egui::Ui) {
        let others: Vec<absentia_ipc::VaultOverview> = self
            .overviews
            .iter()
            .filter(|o| {
                self.vault
                ...
                egui::Frame::none()
                    ...
                    .show(ui, |ui| {
                        ...
                        let cmd = match &self.vault {
                            Some(v) => format!(
                                "absd watch {} --interval 300 --allow-destruction",
                                v.display()
                            ),
                            None => "absd watch <vault> --interval 300 --allow-destruction".into(),
                        };
            ...
            ui.horizontal(|ui| {
                ...
                if enrolled && quiet_button(ui, "Add token\u{2026}").clicked() {
                    if let Some(p) = rfd::FileDialog::new()
                        ...
                        .pick_file()
                    {
                        match absentia_ipc::validate_token_file(&p) {
                            Ok(()) => {
