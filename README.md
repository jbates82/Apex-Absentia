...
- `absentia`, the desktop window
- `abs`, the command line
- `absd`, the background service that watches deadlines
...
```
abs vault create secrets.azv
abs vault import secrets.azv report.pdf notes.txt
abs vault list   secrets.azv
abs vault export secrets.azv ./recovered
abs vault verify secrets.azv
```

Protect it against a stolen drive, then open it:

```
abs split enroll secrets.azv /media/usb/token.txt /media/safe/custodian.txt
abs split status secrets.azv
abs vault list   secrets.azv --token /media/usb/token.txt
```
...
```
abs destroy dry-run   secrets.azv
abs deadman configure secrets.azv --enable --timeout 259200 --heartbeat 86400
absd watch secrets.azv --interval 300
absd watch secrets.azv --interval 300 --allow-destruction
```

`abs` never destroys a vault on a timer. Only `absd` does, and only when you
...
None of this is something Absentia can fix. The file was there before this
program saw it.
...
| | |
| --- | --- |
| `docs/ABOUT-ABSENTIA.txt` | what it does, and how it compares to other tools |
