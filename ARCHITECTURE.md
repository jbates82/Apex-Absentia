...
```
absentia-core            errors, limits, deadman state machine. Depends on nothing of ours.
absentia-secure-memory   SecretBytes, SecureBuffer. Zeroing, redacted Debug, constant-time eq.
absentia-kdf             Argon2id, parameter validation and floors.
absentia-crypto          AEAD suites, HKDF subkeys, nonce construction, Merkle root.
absentia-compress        Zstandard, content classification, bounded decompression.
absentia-format          AZV1 header and manifest encoding and parsing.
absentia-auth            Factor kinds, factor sets, multi-factor KEK composition.
absentia-presence        Signal trust levels, decaying confidence scoring.
absentia-policy          Deadman configuration and state evaluation.
absentia-journal         Hash-chained persistent state journal.
absentia-sanitize        Platform sanitizer trait and portable implementation.
absentia-destroy         Authorization, cryptographic erasure, reporting.
absentia-ipc             The command surface the GUI talks to.
absentia-platform        Filesystem knowledge and service definitions.
absentia-audit           Hash-chained tamper-evident log.
absentia-vault           Lifecycle: create, open, import, export, verify, close.
absentia-cli             The `abs` binary.
```
...
Strictly downward. `absentia-crypto` depends on no GUI, no networking and no
...
`abs service unit` generates a definition for. They are absent rather than stubbed. A
...
`absentia_ipc::Session` and renders what comes back, so the security boundary
...
**Absentia does not install its own service.** Writing to a service directory
