...
#
# The capability table in README.md is a copy of what `abs security audit`
...
"$ZT" security audit | grep -E "^  [A-Z]" | sed -E 's/^  ([A-Za-z].*[^ ])  +([A-Z][A-Z ]*[A-Z])$/\1|\2/' \
    | grep '|' | sort > /tmp/abs-audit-actual.txt

# Every row of the README table.
grep -E '^\| [A-Za-z].*\|.*\|$' README.md \
    | sed -E 's/^\| (.*[^ ]) +\| (.*[^ ]) \|$/\1|\2/' \
    | grep -E '\|(VERIFIED|BEST EFFORT|NOT IMPLEMENTED|NOT SUPPORTED|ENFORCED)$' \
    | sort > /tmp/abs-audit-readme.txt

missing=$(comm -23 /tmp/abs-audit-actual.txt /tmp/abs-audit-readme.txt || true)
extra=$(comm -13 /tmp/abs-audit-actual.txt /tmp/abs-audit-readme.txt || true)
