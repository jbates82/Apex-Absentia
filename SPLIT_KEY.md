...
arrives. Cryptographic erasure only fires if `absd` is running on the original
...
a threshold of two does not, and `abs split status` says `PERMANENT DATA LOSS`
...
```
abs split enroll  vault.azv /media/usb/token.txt /media/safe/custodian.txt
abs split status vault.azv

abs vault list vault.azv --token /media/usb/token.txt
abs vault list vault.azv --token /media/usb/token.txt --token /media/safe/custodian.txt --no-password
```
...
component, stated by `abs split status` rather than left to be discovered: any
