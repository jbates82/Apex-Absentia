...
- **Concurrency.** `absd` exists but is single-threaded and takes no lock, so
  two instances watching one vault, or `abs` and `absd` running together, are not
...
- **The GUI, by automation.** The window is rendered and driven with synthetic
  ...
  covered only by hand. The tested part is `absentia-ipc`, which is where the
