...
Checked before committing this time: nothing in encryption or security carries
the name Absentia.

The name is the mechanism rather than a mood. *In absentia* is judgment passed
on somebody who is not present, which is what a deadman switch does.

What changed:
- Nineteen crates from `zerotrace-*` to `absentia-*`
- The three programs are now `absentia` (the window), `abs` (command line) and
  `absd` (the service)
- Configuration moves from `apex-zerotrace` to `apex-absentia`
- Cryptographic domain separator strings now read `apex-absentia:`, so vaults
  made under the old name will not open. Everything is pre-1.0 and there is no
  migration.

What did not change: the `.azv` extension and the `AZV` container magic, which
still read correctly, and every sidecar name.

Three real bugs surfaced while testing the rename, none of them caused by it:

- `abs duress set` opened the vault directly rather than through the
  token-aware path, so a duress password could not be set on a split-protected
  vault at all.
- The command line's own open path never tried the duress identity on a split
  vault, so a duress password did nothing there. The same fix was made in the
  boundary the window uses at 0.19.3; this path was missed, which is the third
  time a rule written in one place has not reached the other.
- The decoy file list filtered arguments only on a leading `--`, so it swallowed
  the path after `--token`. A vault set up with `abs duress set v.azv decoy.txt
  --token usb/t.txt` would have built a decoy containing the recovery token:
  the one file it must never show. Flags that take a value now consume it.

That last one is the worst thing found in this session. It would have handed an
attacker a key component at exactly the moment the feature existed to protect
somebody.
...
Added to both manuals and `docs/ABOUT-ABSENTIA.txt`:
...
- That the operating system may hold snapshots Absentia cannot reach, that
  `abs platform report` will say so, and why deleting them is not something this
...
feature.** `abs platform report` now says whether the vault sits on a volume
...
`abs security audit` actually prints, and fails when they disagree.
...
`THREAT_MODEL.md` now states what the sidecar names reveal: that Absentia is
...
- `apps/absentia-desktop/README.md` had a whole section explaining why the
...
- the `absentia-ipc` module documentation made the same TypeScript argument
...
- `abs split enrol` still works as a command. Somebody may have it in a script,
...
Added:
- `crates/absentia-ipc/tests/fuzz.rs`, a deterministic mutation fuzzer over
...
Removed:
- `apps/absentia-gui`, the Tauri front end. It was never compiled by anyone,
...
Added:
- `docs/ABOUT-ABSENTIA.txt`, an overview of every feature with a comparison
  against VeraCrypt, BitLocker, Cryptomator, archivers, Shamir tools and
  deadman services, and a plain account of where Absentia is behind them.
...
Added:
- `abs split custody` and `abs split checkin`. A custodian is handed one key
...
Note on spelling: new commands use `enroll` and `enrollment`. `abs split enroll`
...
- Failed attempts are counted from the audit log, so clearing the count means
  editing a hash chain that `abs audit verify` then reports.
...
- `abs deadman checkin`, `abs auth list` and `abs panic destroy` ignored `--token`,
...
"What v0.7 does" at version 0.12.1, said the binary was `target/release/abs`
...
copied from what `abs security audit` prints, so the two can be compared rather
...
Added:
- `absentia-platform::autostart`, and a "Restart at login" panel in the
...
- The system-wide `abs service unit` path remains for machines that should watch
  a vault whether or not anybody logs in.

## 0.11.2 - Windows build fix, and removing the reason it broke

Fixed:
- `absentia-platform` did not compile on Windows. A `HANDLE` in `windows-sys`
...
forbid unsafe entirely; the exception is `absentia-secure-memory`, whose two
...
- `abs --help` still claimed "The deadman mechanism is NOT IMPLEMENTED in v0.1.
...
Added:
- `absentia-platform::watch`: a per-vault watch lock recorded beside the vault
...
definition with `abs service unit`.
...
- A process started by the window would die with the window or at logout,
  ...
  `abs service unit` generates a definition for.
...
- `Session::split_status`, `Session::enroll_split`, `Session::with_tokens` and
  `Session::without_password` in `absentia-ipc`
...
- `ComponentKind::Custodian`: a share held by a second party or in a second
  ...
  the vault.
- `abs split enroll` writes two tokens by default. Enrolling without redundancy
...
two tokens held together are full access without the password. `abs split
status` says so.
...
- CLI: `abs split enroll <vault> <token-file>`, and `--token` on every command
  that opens a vault
...
Stated plainly in the docs and in `abs split explain`:
...
Added:
- `absentia-secure-memory::lock`: `mlock` on Unix, `VirtualLock` on Windows,
...
Fixed:
- `absentia-desktop` carried two identical `Pending::Export | Pending::ExportAll`
...
Fixed:
- **The window could not get files out of a vault.** `absentia-ipc` had an
...
Added:
- A Deadman policy panel in `absentia-desktop`. The timer could previously
...
Fixed:
- `absentia-desktop` declared Wayland crates and Linux display features as
...
Added:
- `apps/absentia-desktop`: an egui/eframe window over `absentia-ipc`, in the
...
Added:
- `absentia-enterprise`: Ed25519-signed remote commands with replay
  resistance, and k-of-n threshold recovery via Shamir sharing
- CLI: `abs enterprise keygen`, `abs recovery explain`
...
Added:
- `absentia-platform`: filesystem identification, filesystem-aware
  sanitization, and service-definition generation for systemd, launchd and
  Windows
- CLI: `abs platform report`, `abs service unit`
...
- The systemd unit is a user unit with `NoNewPrivileges`, `ProtectSystem` and
  ...
  elevated access.
- Absentia does not install service definitions itself. Writing to a service
...
Added:
- `absentia-ipc`: the typed command surface the GUI talks to. Tested.
- `apps/absentia-gui`: Tauri shell and TypeScript front end. **Source only,
...
Treat `apps/absentia-gui` as unverified until it builds on your machine. It is
excluded from the workspace, so the rest of the project is unaffected.

## 0.4.0 - Phase 4, destruction

Added:
- `absentia-sanitize`: platform sanitizer trait with a portable best-effort
  implementation, and STANDARD / ENHANCED / MAXIMUM profiles
- `absentia-destroy`: two-phase authorization, cryptographic erasure,
  resumption after interruption, dry run, and the destruction report
- `absd`, a resident service that evaluates the policy on a timer, resumes an
...
Still NOT IMPLEMENTED:
- OS service manager integration, so nothing restarts `absd` if it dies
...
Added:
- `absentia-core::time`: wall-clock deadlines cross-checked against monotonic
  time, clock-anomaly detection
- `absentia-presence`: trust-levelled signals, decaying confidence, and a hard
  ceiling on what non-strong evidence can contribute
- `absentia-policy`: deadman configuration with safety floors
- `absentia-journal`: hash-chained persistent state journal that also anchors
  the audit log
- CLI: `abs deadman status|checkin|configure`, `abs journal verify`
...
Added:
- `absentia-auth`: factor kinds, factor sets, multi-factor KEK composition
- AZV format version 2, with the required-factor set inside the authenticated
  header region. Version 1 vaults still open.
- `absentia-audit`: hash-chained tamper-evident log
- CLI: `abs auth list`, `abs audit show`, `abs audit verify`
...
- `abs` CLI: vault create, status, list, verify, import, export; security audit
