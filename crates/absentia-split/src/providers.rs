...
/// that composition already exists in `absentia-auth` and is applied before
...
impl UserProvider {
    pub fn from_password(password: &[u8], salt: &[u8], params: KdfParams) -> Result<Self> {
        ...
        let key = crypto::derive_subkey(&derived, salt, b"apex-absentia:split:user-component")?;
        Ok(Self { key })
    }
}
...
impl RecoveryToken {
    ...
    pub fn encode(&self) -> String {
        ...
        format!("apex-absentia-recovery-token:v1:{hex}")
    }

    pub fn decode(text: &str) -> Result<Self> {
        let hex = text
            .trim()
            .strip_prefix("apex-absentia-recovery-token:v1:")
            .ok_or_else(|| Error::Format("not a Absentia recovery token".into()))?;
