...
use absentia_core::{Error, Result, VaultId};
use absentia_crypto::CryptoSuite;
use absentia_auth::{AuthDescriptor, FactorSet};
...
impl DecoySlot {
    ...
    pub fn random() -> Self {
        ...
        absentia_crypto::random_bytes(&mut key_nonce);
        absentia_crypto::random_bytes(&mut wrapped_key);
        absentia_crypto::random_bytes(&mut kdf_salt);
        let mut off = [0u8; 8];
        let mut len = [0u8; 8];
        absentia_crypto::random_bytes(&mut off);
        absentia_crypto::random_bytes(&mut len);
        let mut boff = [0u8; 8];
        let mut blen = [0u8; 8];
        absentia_crypto::random_bytes(&mut boff);
        absentia_crypto::random_bytes(&mut blen);
        DecoySlot {
            key_nonce,
            wrapped_key,
            kdf_salt,
    ...
    pub fn encode(&self) -> [u8; DECOY_BLOCK_LEN] {
        ...
        absentia_crypto::random_bytes(&mut b[136..]);
        b
    }
    ...
    pub fn from_bytes(b: &[u8]) -> Result<Self> {
        ...
        if b[0..8] != MAGIC {
            return Err(Error::Format(
                "not an Apex Absentia vault (magic number does not match)".into(),
            ));
        }
        ...
        if manifest_len > absentia_core::limits::MAX_MANIFEST_BYTES {
