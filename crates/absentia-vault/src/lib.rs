...
//!
//! ```text
//! [0..256)                 header (see absentia-format)
...
use absentia_compress::{self as compress, CompressSuite};
use absentia_core::{limits, Assurance, Error, Result, VaultId};
use absentia_crypto::{self as crypto, CryptoSuite};
use absentia_format::manifest::{ChunkRef, Entry, Manifest};
use absentia_format::{DecoySlot, Header, DECOY_BLOCK_LEN, HEADER_LEN};
use absentia_auth::{compose_kek, AuthDescriptor, Authenticator, FactorKind, FactorSet};
use absentia_kdf::{derive_kek, KdfParams};
use absentia_split::{seal as seal_split, unseal as unseal_split, ComponentKind, SplitBundle};
use absentia_secure_memory::Key256;
...
fn manifest_aad(vault_id: &VaultId, len: u64, generation: u64) -> Vec<u8> {
    ...
    aad.extend_from_slice(b"apex-absentia:manifest:v2");
...
impl Vault {
    ...
    pub fn is_split_protected(&self) -> bool {
        self.header.flags & absentia_format::flags::SPLIT_PROTECTED != 0
    }
    ...
    pub fn create_with<P: AsRef<Path>>(
        ...
        let mut kdf_salt = [0u8; absentia_format::SALT_LEN];
        ...
        if wrapped.len() != absentia_format::WRAPPED_KEY_LEN {
        ...
        let mut v = Vault {
            ...
            data_end: absentia_format::V3_DATA_START,
    ...
    pub fn open_with<P: AsRef<Path>>(
        ...
        if header.flags & absentia_format::flags::SPLIT_PROTECTED != 0 {
            ...
            return Err(Error::Other(
                "this vault is protected by split keys and cannot be opened with a password \
                 alone. Supply the required key components; see `abs split status`"
                    .into(),
            ));
        }
    ...
    pub fn enroll_split(
        ...
        self.header.flags |= absentia_format::flags::SPLIT_PROTECTED;
        ...
        let _ = absentia_core::restrict_permissions(&bundle_path);
        self.save()?;
        Ok(bundle)
    }
    ...
    fn try_duress<P: AsRef<Path>>(path: P, password: &[u8]) -> Result<Option<Vault>> {
        ...
        let mut shredded = [0u8; absentia_format::WRAPPED_KEY_LEN];
        ...
        let mut readback = [0u8; absentia_format::WRAPPED_KEY_LEN];
        f.seek(SeekFrom::Start(104))?;
    ...
    pub fn set_duress_password_with_progress(
        ...
        let shift = append_at - absentia_format::V3_DATA_START;
        ...
        if decoy_wrapped.len() != absentia_format::WRAPPED_KEY_LEN {
            return Err(Error::Crypto("unexpected wrapped key length".into()));
        }
        let mut decoy_wrapped_arr = [0u8; absentia_format::WRAPPED_KEY_LEN];
        ...
        let body_start = absentia_format::V3_DATA_START as usize;
    ...
    pub fn export_with_progress<P: AsRef<Path>>(
        ...
        if let Some(parent) = out_path.parent() {
            ...
            absentia_core::refuse_symlinked_path(parent)?;
        }
        absentia_core::refuse_symlinked_path(&out_path)?;
        ...
        let _ = absentia_core::restrict_permissions(&part_path);
...
fn decoy_aad(vault_id: &absentia_core::VaultId) -> Vec<u8> {
    let mut aad = Vec::with_capacity(48);
    aad.extend_from_slice(b"apex-absentia:decoy-wrap:v1");
    aad.extend_from_slice(vault_id.as_bytes());
    aad
}
