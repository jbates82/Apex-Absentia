...
fn tampering_with_the_header_is_detected() {
    ...
    for i in 0..absentia_format::HEADER_AAD_LEN {
...
fn corrupting_a_chunk_is_detected_rather_than_returned() {
    ...
    raw[absentia_format::V3_DATA_START as usize + 40] ^= 0x40;
...
fn both_crypto_suites_work() {
    for suite in [
        absentia_crypto::CryptoSuite::XChaCha20Poly1305,
        absentia_crypto::CryptoSuite::Aes256Gcm,
    ] {
...
fn export_refuses_a_path_that_escapes_the_directory() {
    use absentia_vault::sanitize_relative_path;
...
use absentia_auth::{FactorKind, FactorSet, SoftwareAuthenticator};
...
fn a_two_factor_vault_needs_both_factors() {
    ...
    match Vault::open_with(&vp, b"pw", None) {
        Err(absentia_core::Error::NotImplemented(_)) => {}
...
fn every_bit_of_the_auth_region_is_authenticated() {
    ...
    for i in absentia_format::AUTH_REGION {
        let mut bad = good.clone();
        bad[i] ^= 1;
        std::fs::write(&vp, &bad).unwrap();
        assert!(
...
fn a_real_fido_authenticator_refuses_rather_than_failing_open() {
    ...
    let vp = dir.join("v.azv");
    let real = absentia_auth::Fido2Authenticator;
...
fn version_one_vaults_still_open() {
    ...
    assert_eq!(v.format_version(), absentia_format::FORMAT_VERSION);
    v.close();
...
use absentia_split::{ComponentKind, RecoveryToken, UserProvider};

fn user_component(password: &[u8], salt: &[u8]) -> absentia_secure_memory::Key256 {
    UserProvider::from_password(
        password,
        salt,
        absentia_kdf::KdfParams { memory_kib: 19 * 1024, time_cost: 2, parallelism: 1 },
    )
    .unwrap()
    .key()
    .unwrap()
...
fn enrolling_split_protection_does_not_re_encrypt_the_contents() {
    ...
    let chunk_start = absentia_format::HEADER_LEN;
    let window = 64 * 1024;
...
fn deleting_the_split_bundle_denies_service_rather_than_downgrading() {
    ...
    std::fs::remove_file(absentia_vault::split_bundle_path(&vp)).unwrap();
...
fn a_bundle_from_another_vault_is_refused() {
    ...
    std::fs::copy(
        absentia_vault::split_bundle_path(&b),
        absentia_vault::split_bundle_path(&a),
    )
    .unwrap();
...
fn a_split_bundle_is_not_world_readable() {
    ...
    let mode = std::fs::metadata(absentia_vault::split_bundle_path(&vp))
        .unwrap()
        .permissions()
        .mode();
...
fn an_older_manifest_cannot_be_put_back() {
    ...
    let start = absentia_format::Header::from_bytes(&old).unwrap().manifest_offset as usize;
