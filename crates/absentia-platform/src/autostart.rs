...
use absentia_core::{Error, Result};
...
fn tag(vault: &Path) -> String {
    let digest = absentia_crypto::sha256(vault.display().to_string().as_bytes());
...
pub fn entry_path(vault: &Path) -> Result<PathBuf> {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        Ok(dir.join(format!("apex-absentia-{t}.cmd")))
    }
    #[cfg(target_os = "macos")]
    {
        Ok(dir.join(format!("com.apex.absentia.{t}.plist")))
    }
    #[cfg(all(unix, not(target_os = "macos")))]
    {
        Ok(dir.join(format!("apex-absentia-{t}.desktop")))
    }
    #[cfg(not(any(unix, target_os = "windows")))]
    {
        let _ = (dir, t);
...
impl AutostartPlan {
    pub fn new(absd: &Path, vault: &Path) -> Self {
        Self {
            absd: absd.to_path_buf(),
    ...
    pub fn render(&self) -> String {
        #[cfg(target_os = "windows")]
        {
            format!(
                "@echo off\r\n\
                 REM Apex Absentia watcher, started when this user logs in.\r\n\
                 REM Delete this file to stop it starting.\r\n\
                 REM Destruction permitted: {}\r\n\
                 start \"Apex Absentia\" /MIN \"{}\" {}\r\n",
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            format!(
                "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n\
                 ...
                 <plist version=\"1.0\">\n<dict>\n\
                 \x20 <key>Label</key>\n  <string>com.apex.absentia.{}</string>\n\
...
mod tests {
    ...
    fn plan(dir: &Path, allow: bool) -> AutostartPlan {
        let absd = dir.join("absd");
        std::fs::write(&absd, b"#!/bin/sh\n").unwrap();
        let mut p = AutostartPlan::new(&absd, &dir.join("v.azv"));
    ...
    fn destruction_is_off_by_default() {
        let d = tmp("default");
        let p = AutostartPlan::new(&d.join("absd"), &d.join("v.azv"));
    ...
    fn installing_without_the_service_program_is_refused() {
        let d = tmp("missing");
        let p = AutostartPlan::new(&d.join("nonexistent-absd"), &d.join("v.azv"));
