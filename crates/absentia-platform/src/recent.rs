...
fn config_dir() -> Option<PathBuf> {
    ...
    {
        std::env::var_os("APPDATA").map(|a| PathBuf::from(a).join("ApexAbsentia"))
    }
    #[cfg(target_os = "macos")]
    {
        std::env::var_os("HOME")
            .map(|h| PathBuf::from(h).join("Library").join("Application Support").join("ApexAbsentia"))
    }
