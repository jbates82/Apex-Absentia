...
//! belongs to the operating system's supervisor, not to Absentia.
//!
...
/// Supervisors Absentia can generate definitions for.
...
impl Supervisor {
    ...
    pub fn install_hint(&self) -> &'static str {
        match self {
            Supervisor::Systemd => "~/.config/systemd/user/absentia.service",
            Supervisor::Launchd => "~/Library/LaunchAgents/com.apex.absentia.plist",
...
impl ServicePlan {
    ...
    fn systemd(&self) -> String {
        ...
        format!(
            "[Unit]\n\
             Description=Apex Absentia deadman service\n\
             Documentation=man:absd(1)\n\
    ...
    fn launchd(&self) -> String {
        ...
        format!(
            "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n\
             ...
             <dict>\n\
             \x20 <key>Label</key>\n\
             \x20 <string>com.apex.absentia</string>\n\
    ...
    fn windows(&self) -> String {
        ...
        format!(
            "REM Register the Apex Absentia service. Run from an elevated prompt.\n\
             REM Review the arguments before running: {destruction}\n\
             \n\
             sc.exe create ApexAbsentia ^\n\
             \x20 binPath= \"{exe} {args}\" ^\n\
             \x20 start= auto ^\n\
             \x20 DisplayName= \"Apex Absentia deadman service\"\n\
             \n\
             sc.exe failure ApexAbsentia reset= 86400 actions= restart/10000/restart/10000/restart/10000\n\
             sc.exe description ApexAbsentia \"Evaluates the Absentia deadman policy. Holds no vault keys.\"\n",
            exe = self.executable,
            args = self.args(),
...
/// Whether a supervisor is actually managing Absentia right now.
///
...
mod tests {
    ...
    fn plan() -> ServicePlan {
        ServicePlan::new(Path::new("/usr/local/bin/absd"), Path::new("/home/u/vaults/v.azv"))
    }
    ...
    fn the_launchd_plist_is_well_formed_and_keeps_alive() {
        ...
        assert!(text.contains("com.apex.absentia"));
    ...
    fn the_windows_script_configures_restart_on_failure() {
        ...
        assert!(text.contains("sc.exe create ApexAbsentia"));
