...
use absentia_auth::audit_events as ev;
use absentia_core::state::DeadmanState;
use absentia_core::time::{self, TimeAnchor};
use absentia_core::{Assurance, Error, Result, VaultId};
...
impl Session {
    ...
    fn split_path(&self) -> PathBuf {
        absentia_vault::split_bundle_path(&self.vault_path)
    }
    ...
    pub fn is_split_protected(&self) -> bool {
        ...
        absentia_format::Header::from_bytes(&hb)
            .map(|h| h.flags & absentia_format::flags::SPLIT_PROTECTED != 0)
            .unwrap_or(false)
    }
    ...
    fn header_kdf(&self) -> Result<([u8; 32], absentia_kdf::KdfParams)> {
        use std::io::Read;
    ...
    pub fn watch_status(&self) -> WatchView {
        use absentia_platform::watch::{status, WatchStatus};
    ...
    pub fn stop_watch(&self) -> Result<()> {
        absentia_platform::watch::request_stop(&self.vault_path)
    }

    pub fn clear_stale_watch(&self) -> Result<()> {
        absentia_platform::watch::clear_stale(&self.vault_path)
    }
    ...
    /// means editing a hash chain that `abs audit verify` then reports.
    ...
    /// chain, which `abs audit verify` would then report.
    ...
    fn fetch_custodian_component(
        &self,
        password: &str,
    ) -> Result<Option<absentia_secure_memory::Key256>> {
        use absentia_remote::custodian::{Custodian, DirectoryCustodian};
        use absentia_remote::{signing_identity, Intent, Request, Verdict};
        ...
        match custodian.handle(&request, now)? {
            Verdict::Released(bytes) => {
                ...
                let mut k = absentia_secure_memory::Key256::zeroed();
    ...
    pub fn set_duress_password_with_progress(
        ...
        absentia_kdf::strength::require_acceptable(duress_password)?;
    ...
    pub fn establish_custody(
        ...
        use absentia_remote::verifying_key;
    ...
    pub fn set_custodian(&self, dir: &Path) -> Result<()> {
        ...
        let _ = absentia_core::restrict_permissions(&self.custodian_path());
    ...
    pub fn autostart_status(&self) -> AutostartView {
        use absentia_platform::autostart;
    ...
    pub fn install_autostart(
        ...
        use absentia_platform::autostart::{install, AutostartPlan};
        let absd = self.service_program()?;
        let mut plan = AutostartPlan::new(&absd, &self.vault_path);
    ...
    pub fn remove_autostart(&self) -> Result<()> {
        absentia_platform::autostart::remove(&self.vault_path)?;
    ...
    fn service_program(&self) -> Result<PathBuf> {
        ...
        let absd = dir.join(if cfg!(windows) { "absd.exe" } else { "absd" });
        if !absd.exists() {
    ...
    pub fn enroll_split(
        ...
        let _ = absentia_core::restrict_permissions(token_path);
        if let (Some(p), Some(c)) = (custodian_path, &custodian) {
            std::fs::write(p, format!("{}\n", c.encode()))?;
            let _ = absentia_core::restrict_permissions(p);
        }
    ...
    pub fn export_with_progress(
        ...
        // exists to prevent.
        let under_duress = v.opened_as() == absentia_vault::Identity::Decoy;
    ...
    fn header_vault_id(&self) -> Option<VaultId> {
        ...
        absentia_format::Header::from_bytes(&hb).ok().map(|h| h.vault_id)
    }
    ...
    pub fn deadman_status(&self) -> Result<DeadmanStatus> {
        ...
        if !self.journal_describes_this_vault(&journal) {
            ...
            return Ok(DeadmanStatus {
                ...
                recorded_state: absentia_core::state::DeadmanState::Normal.label().into(),
                confidence,
...
pub fn capabilities() -> Vec<CapabilityRow> {
    ...
    let (mem, mem_note) = absentia_secure_memory_status();
    vec![
...
fn absentia_secure_memory_status() -> (Assurance, &'static str) {
    absentia_secure_memory::probe_memory_protection()
}
...
/// Whether a file is a usable Absentia token.
///
