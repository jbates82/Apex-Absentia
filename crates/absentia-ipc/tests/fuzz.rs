...
use absentia_compress::CompressSuite;
use absentia_format::manifest::{ChunkRef, Entry, Manifest};
use absentia_format::Header;
...
fn valid_header() -> Vec<u8> {
    ...
    absentia_vault::Vault::create(
        &path,
        b"correct-horse-battery-staple",
        &absentia_vault::VaultOptions::default(),
    )
    .unwrap()
    .close();
    let bytes = std::fs::read(&path).unwrap();
    let _ = std::fs::remove_dir_all(&dir);
    bytes[..absentia_format::HEADER_LEN].to_vec()
}
...
fn the_header_parser_survives_arbitrary_bytes() {
    let corpus = vec![
        valid_header(),
        vec![0u8; absentia_format::HEADER_LEN],
...
fn the_split_bundle_parser_survives_arbitrary_bytes() {
    use absentia_crypto::{random_key, CryptoSuite};
    use absentia_split::{seal, ComponentKind, SplitBundle};

    let bundle = seal(
        &random_key(),
        absentia_core::VaultId::from_bytes([3u8; 16]),
...
fn the_token_parser_survives_arbitrary_bytes() {
    use absentia_split::RecoveryToken;
    let corpus = vec![
        RecoveryToken::generate().encode().into_bytes(),
        b"apex-absentia-recovery-token:v1:".to_vec(),
...
fn the_compressor_survives_arbitrary_bytes() {
    ...
    hammer("zstd", 0x5EED_0005, corpus, 10_000, |b| {
        let _ = absentia_compress::decompress(CompressSuite::Zstd, b, 4096);
        let _ = absentia_compress::decompress(CompressSuite::Store, b, b.len());
    });
}

fn zstd_encoded() -> Vec<u8> {
    let (_, c) =
        absentia_compress::compress(&b"repeating text ".repeat(200), 3, CompressSuite::Zstd)
            .unwrap();
    c
}
...
fn the_custodian_record_parser_survives_arbitrary_bytes() {
    ...
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
    let id = absentia_core::VaultId::from_bytes([4u8; 16]);
...
fn a_parser_never_allocates_on_a_length_it_read() {
    ...
    assert!(absentia_split::SplitBundle::decode(&bundle).is_err());

    // And a chunk claiming more plaintext than the absolute cap.
    assert!(absentia_compress::decompress(
        CompressSuite::Zstd,
        &zstd_encoded(),
        usize::MAX / 2
    )
