...
use absentia_ipc::{capabilities, Session, DESTROY_CONFIRMATION};
...
fn a_wrong_file_is_rejected_when_it_is_chosen() {
    ...
    assert!(absentia_ipc::validate_token_file(&junk).is_err());
    assert!(absentia_ipc::validate_token_file(&dir.join("absent.txt")).is_err());

    let s = seeded(&dir);
    let token = dir.join("token.txt");
    s.enroll_split(PW, &token, None).unwrap();
    assert!(absentia_ipc::validate_token_file(&token).is_ok());
}
...
fn destruction_takes_the_split_bundle_with_it() {
    ...
    let bundle = absentia_vault::split_bundle_path(&dir.join("v.azv"));
...
fn an_existing_vault_still_opens_with_a_password_below_the_floor() {
    ...
    let dir = tmp("oldpw");
    let s = Session::new(dir.join("v.azv"));
    absentia_vault::Vault::create(&dir.join("v.azv"), b"short", &VaultOptions::default())
        .unwrap()
        .close();
...
fn backoff_grows_and_is_capped() {
    use absentia_ipc::backoff_seconds;
