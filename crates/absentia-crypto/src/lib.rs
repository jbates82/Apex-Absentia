...
pub mod info {
    pub const METADATA: &[u8] = b"apex-absentia:v1:metadata-key";
    pub const FILE: &[u8] = b"apex-absentia:v1:file-key";
    pub const INTEGRITY: &[u8] = b"apex-absentia:v1:integrity-key";
}
...
pub fn merkle_root(leaves: &[[u8; 32]]) -> [u8; 32] {
    if leaves.is_empty() {
        return sha256(b"apex-absentia:v1:empty-tree");
    }
    ...
    while level.len() > 1 {
        ...
        for pair in level.chunks(2) {
            let mut h = Sha256::new();
            h.update(b"apex-absentia:v1:node");
            h.update(pair[0]);
